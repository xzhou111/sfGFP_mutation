...
<br>
  ...
  2.model:按照下面网址，提前下载蛋白模型ESM，安装ProteinMPNN<br>
  ```bash
...
<br>
  3.candidates：<br>
    ...
    python script/bin/01_make_fixed_jsonl.py    #得到可识别的fixed_positions.jsonl文件<br>
  ```python
  ...
  ```
    ...
    ```
    #对ProteinMPNN生成的 sfGFP 候选序列进行 ESM-2 打分，然后按分数从高到低排序，筛出 Top 候选序列<br>
    ```python
      python script/bin/02_esm2_score_top.py
    ```

  4.filter：再通过pLDDT和RMSD进一步筛选<br>
    #预测蛋白结构(也可以通过alphafold3预测)<br>
    ```bash
